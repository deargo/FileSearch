...
void DialogHelp::showUsage()
{
    ...
    addText("搜索条件","包含目录、排除目录、包含文件和排除文件，不支持路径。");
    addText("列表选择","支持单击、双击、Ctrl和Shift键操作。");
    addText("删除操作","支持列表选择项的：从列表删除、彻底删除，和删除到回收站。");
    addText("导出结果","导出列表中的全部数据为CSV文件，到指定目录。");
    addText("多个操作","支持多个目标，多个条件的操作，但需使用英文分号[;]隔开。");
    addBlank();
    addAuthor();
}
...
{
    addText("工具简介","Windows文件搜索工具。支持指定目录和文件，并按照文件名，或文件内容的进行搜索。对于搜索结果，支持删除列表、彻底删除、删除到回收站，和导出CSV操作。");
}
